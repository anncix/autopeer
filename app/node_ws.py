...
Architecture:
  NodeHub      → tracks live agent connections, handles register/unregister/request
  NodeConnection → single agent socket, handles send/close/pending futures
...
class NodeConnection:
    """Single agent WebSocket connection with request/response correlation."""
...
class NodeHub:
    """Central registry for all connected node agents.
    ...
    def _mark_online(
        ...
        """Refresh in-memory liveness for a still-current connection.

        No DB I/O: this runs on the command hot path, so persistence is left to the
...
async def request_node(
    ...
    """Send a command to a node agent and await its response (async context)."""
...
def request_node_sync(
    ...
    """Send a command to a node agent from synchronous code.

    Uses anyio.from_thread when inside an async worker thread, otherwise
...
def node_runtime_context(nodes: list[Node]) -> dict[str, dict[str, Any]]:
    """Build the per-node runtime context used by templates."""
