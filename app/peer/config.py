...
def render_bird_peer_config(peer: PeerRequest, node: Node, local_asn: str) -> str:
    ...
    lines = [
        ...
        f"# The dnpeers template wires import filter dn42_import / export filter dn42_export",
        f"# (ROA-validated, with DN42 standard BGP communities). See app/peer/bird_base.py",
        f"# or the 'Export DN42 base config' page for the template definition to include.",
        f"# dnpeers 模板已接好 import filter dn42_import / export filter dn42_export",
        f"# (ROA 驗證 + DN42 標準 BGP communities)。模板定義見 app/peer/bird_base.py",
        f"# 或「匯出 DN42 基礎設定」頁面,將其 include 至主 bird.conf。",
        f"# DN42 communities for this peer: ({local_asn_number}, 33) crypto=strong (WireGuard)",
        "",
