...
payload assembly, synchronous dispatch, or result application.
"""
...
class PeerDeployError(Exception):
    """Raised when deployment prerequisites are missing or the node rejects config."""


def build_deploy_payload(peer: PeerRequest, node: Node, settings: Settings) -> dict[str, Any]:
    """Assemble the full WireGuard + BIRD configuration payload for a node."""
...
def deploy_peer(
    ...
    """Send a deploy command to the node agent and return its response."""
    if not node.enabled:
...
def fetch_node_public_key(node: Node, timeout: float = 10.0) -> str | None:
    """Fetch and validate the node's own WireGuard public key.
...
def apply_deploy_result(peer: PeerRequest, result: dict[str, Any]) -> None:
    """Write the deployment result back onto the peer row in the current session."""
